...
def loss_function(recon_x, x, tab_pred, tab_labels, img_pred, img_labels):
    ...
    tab_loss = F.binary_cross_entropy_with_logits(tab_pred, tab_labels)
...
def test(model, test_data_loader, epoch, best_accuracy, best_auc, best_epoch):
    ...
    with torch.no_grad():
        for tab_data, tab_label, img_data, img_label in test_data_loader:
            ...
            tab_label_indices = torch.argmax(tab_label, dim=1)
...
for epoch in range(1, EPOCH + 1):
    ...
    best_accuracy, best_auc, best_epoch = test(cvae, test_synchronized_loader, epoch, best_accuracy, best_auc, best_epoch)
