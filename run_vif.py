...
def load_dataset(file_path):
    ...
    if file_ext == '.csv':
        ...
        try:
            ...
            # Convert byte strings to regular strings (common in ARFF)
            for col in df.columns:
                if df[col].dtype == 'object':
                    try:
                        df[col] = df[col].str.decode('utf-8')
                    except AttributeError:  # Already string
                        pass
            ...
            print(f"[INFO] Trying alternative ARFF parser...")
            try:
...
if df.shape[1] < 2:
    raise ValueError(f"Dataset has only {df.shape[1]} column(s), need at least 2 (features + target)")
...
# If no named column found AND columns are integers (unnamed), use last column
if target_col is None:
    if all(isinstance(col, int) for col in df.columns):
        target_col = df.columns[-1]
        print(f"[INFO] No named target found. Using last column (index {target_col}) as target.")
    else:
        # For named columns, still default to last
        target_col = df.columns[-1]
        print(f"[INFO] No standard target name found. Using '{target_col}' as target.")

print(f"[INFO] Target column: {target_col}")

# ========== STEP 1: Drop columns with >50% missing (PAPER METHODOLOGY) ==========
...
# ========== VALIDATE: At least one feature remains ==========
if df.shape[1] <= 1:  # Only target remains
    raise ValueError("All feature columns were dropped due to missing data (>50%). Dataset unusable.")
...
if num_classes > 20:
    print(f"[ERROR] Dataset has {num_classes} classes (>20). Table2Image only supports ≤20 classes.")
    print(f"[INFO] Skipping this dataset...")
    exit(1)

if num_classes < 2:
    raise ValueError(f"Dataset has only {num_classes} class. Need at least 2 for classification.")

# ========== STEP 2: Extract features and encode categorical ==========
...
# ========== STEP 3: Impute with MEDIAN (PAPER METHODOLOGY) ==========
...
# ========== CALCULATE VIF (ONE-TIME, PAPER METHODOLOGY) ==========
...
train_synchronized_dataset = SynchronizedDataset(
    train_filtered_tab_set, 
    train_filtered_img_set
)
test_synchronized_dataset = SynchronizedDataset(
    test_filtered_tab_set, 
    test_filtered_img_set
)

train_synchronized_loader = DataLoader(
    dataset=train_synchronized_dataset, 
    batch_size=BATCH_SIZE, 
    shuffle=True
)
test_synchronized_loader = DataLoader(
    dataset=test_synchronized_dataset, 
    batch_size=BATCH_SIZE
)
...
class CVAEWithTabEmbedding(nn.Module):
    def __init__(self, input_dim, tab_latent_size, num_classes, latent_size=8, vif_values=None):
        ...
        # Create VIF model if values provided
...
# ========== TESTING FUNCTION ==========
def test(model, test_data_loader, epoch, best_accuracy, best_auc, best_epoch, best_model_path):
    model.eval()
    test_loss = 0
    correct_tab_total = 0
    correct_img_total = 0
    ...
    with torch.no_grad():
        for tab_data, tab_label, img_data, img_label in test_data_loader:
            ...
            test_loss += loss_function(
                recon_x, img_data, tab_pred, tab_label, img_pred, img_label
            ).item()
        ...
        try:
            if num_classes == 2:
                ...
                tab_auc = roc_auc_score(
                    all_tab_labels_arr, all_tab_preds_arr, 
        ...
        try:
            if num_classes == 2:
                ...
                img_auc = roc_auc_score(
                    all_img_labels_arr, all_img_preds_arr, 
                    multi_class="ovr", average="macro"
                )
        except Exception as e:
